...
class TarefaService {
  ...
  async buscarGabarito(tarefaId) {
    ...
    tarefa.questoes.forEach(q => {

      const alternativaCorreta = q.alternativas.find(a => a.correta === true);

      resultado.questoes.push({
        questaoId: q._id,
        alternativaCorretaId: alternativaCorreta?._id
      });
